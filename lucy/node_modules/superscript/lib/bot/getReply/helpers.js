...
const findMatchingGambitsForMessage = (() => {
  var _ref = _asyncToGenerator(function* (type, id, message, options) {
    let gambitsParent = {};
    const chatSystem = options.system.chatSystem;

    if (type === 'topic') {
      debug.verbose('Looking back Topic', id);
      gambitsParent = yield chatSystem.Topic.findById(id, 'gambits').populate({ path: 'gambits', populate: { path: 'replies' } });
    } else if (type === 'reply') {
      options.topic = 'reply';
      debug.verbose('Looking back at Conversation', id);
      gambitsParent = yield chatSystem.Reply.findById(id, 'gambits').populate({ path: 'gambits', populate: { path: 'replies' } });
    } else {
      throw new Error('We should never get here');
    }

    const matches = yield Promise.all(gambitsParent.gambits.map((() => {
...
const doesMatchTopic = exports.doesMatchTopic = (() => {
  var _ref4 = _asyncToGenerator(function* (topicName, message, options) {
    const topic = yield options.chatSystem.Topic.findOne({ name: topicName }, 'gambits').populate('gambits');

    return yield Promise.all(topic.gambits.map((() => {
...
const eachGambitHandle = (() => {
  var _ref6 = _asyncToGenerator(function* (gambit, message, options) {
    ...
    if (gambit.redirect !== '') {
      ...
      const redirectedGambit = yield chatSystem.Gambit.findOne({ input: gambit.redirect }).populate({ path: 'replies' });
...
const walkGambitParent = (() => {
  var _ref9 = _asyncToGenerator(function* (gambitId, chatSystem) {
    const gambitIds = [];
    try {
      const gambit = yield chatSystem.Gambit.findById(gambitId).populate('parent');
...
const walkReplyParent = (() => {
  var _ref10 = _asyncToGenerator(function* (replyId, chatSystem) {
    const replyIds = [];
    try {
      const reply = yield chatSystem.Reply.findById(replyId).populate('parent');
...
const getRootTopic = (() => {
  var _ref11 = _asyncToGenerator(function* (gambit, chatSystem) {
    if (!gambit.parent) {
      return yield chatSystem.Topic.findOne({ gambits: { $in: [gambit._id] } });
    }

    const gambits = yield walkGambitParent(gambit._id, chatSystem);
    if (gambits.length !== 0) {
      return yield chatSystem.Topic.findOne({ gambits: { $in: [gambits.pop()] } });
    }

    return yield chatSystem.Topic.findOne({ name: 'random' });
  });
