    ...
    const bits = yield Promise.all(potentialReplies.map((() => {
      var _ref2 = _asyncToGenerator(function* (potentialReply) {
        ...
        if (potentialReply.reply.filter) {
          ...
          const cleanFilter = yield new Promise(function (resolve) {
            _processTags2.default.preprocess(potentialReply.reply.filter, stars, options, function (err, cleanFilter) {
              resolve(cleanFilter);
            });
          });
