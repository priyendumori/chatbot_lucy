...
const topicItorHandle = (() => {
  var _ref3 = _asyncToGenerator(function* (topicData, messageObject, options) {
    ...
    if (topicData.type === 'TOPIC') {
      const topic = yield system.chatSystem.Topic.findById(topicData.id).populate('gambits');
      if (topic) {
        ...
        return yield _helpers2.default.findMatchingGambitsForMessage('topic', topic._id, messageObject, options);
      }
      // We call back if there is no topic Object
      // Non-existant topics return false
      return false;
    } else if (topicData.type === 'REPLY') {
      const reply = yield system.chatSystem.Reply.findById(topicData.id).populate('gambits');
      debug.verbose('Conversation reply thread: ', reply);
      if (reply) {
        return yield _helpers2.default.findMatchingGambitsForMessage('reply', reply._id, messageObject, options);
      }
      return false;
    }
