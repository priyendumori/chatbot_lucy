...
const debug = (0, _debugLevels2.default)('SS:ProcessTags'); // TODO: Fix this documentation, options is incorrect
/**
...
const processWordnetLookup = function processWordnetLookup(tag, replyObj, options, callback) {
  ...
  _wordnet2.default.lookup(tag.term, '~', (err, words) => {
    if (err) {
      console.error(err);
    }
...
const preprocess = function preprocess(reply, replyObj, options, callback) {
  ...
  _async2.default.map(captureTags, (tag, next) => {
...
const processTopicRedirect = function processTopicRedirect(tag, replyObj, options, callback) {
  ...
  options.depth += 1;
  return (0, _topicRedirect2.default)(topicName, topicTrigger, options, postAugment(replyObj, tag, callback));
};
...
const processRespond = function processRespond(tag, replyObj, options, callback) {
  ...
  options.depth += 1;
  return (0, _respond2.default)(topicName, options, postAugment(replyObj, tag, callback));
};
...
const processCustomFunction = function processCustomFunction(tag, replyObj, options, callback) {
  ...
  return (0, _customFunction2.default)(tag.functionName, cleanArgs, replyObj, options, (err, response) => {
    ...
    preprocess(response, replyObj, options, (err, preprocessed) => {
      ...
      return _async2.default.mapSeries(replyTags, (tag, next) => {
        if (typeof tag === 'string') {
          next(null, tag);
        } else {
...
const processReplyTags = function processReplyTags(replyObj, options, callback) {
  ...
  preprocess(replyString, replyObj, options, (err, preprocessed) => {
    ...
    _async2.default.mapSeries(replyTags, (tag, next) => {
      if (typeof tag === 'string') {
        next(null, tag);
      } else {
        processTag(tag, replyObj, options, next);
